...
test('주식 게임은 2배 투자와 4일 결산을 처리한다', async () => {
  ...
  try {
    ...
    assert.equal((await emit(players[0], 'stocks:trade', {symbol, side:'buy', amount:200_000_000})).ok, true);
    assert.match((await emit(players[0], 'stocks:trade', {symbol, side:'buy', amount:10_000})).error, /2배/);
